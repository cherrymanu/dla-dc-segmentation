...
\begin{abstract}
This paper presents a divide-and-conquer (D\&C) algorithm for page layout segmentation that recursively partitions a document page using feature-guided XY-cuts and then performs a bottom-up greedy merge of adjacent small regions. The algorithm outputs a hierarchical segmentation tree and labeled regions (text, table, figure, blank). Termination is proven and time complexity is analyzed under mild assumptions on split balance. The algorithm was tested on real document images, and experiments on synthetic pages with known ground truth demonstrate near $O(N\log N)$ scaling with image size and show good agreement between empirical and theoretical behavior.
\end{abstract}
...
Document layout segmentation decomposes a page into semantically meaningful regions such as text, tables, figures, and whitespace. This work explores a D\&C approach that recursively subdivides the page into smaller regions based on feature homogeneity, computed from projection profiles (identifying whitespace valleys and content peaks), edge density, and periodicity measures, until each region is sufficiently uniform according to predefined thresholds. Region boundaries are refined to better align with content, and adjacent leaf regions with compatible classifications are merged in a multi-pass post-processing step to avoid over-segmentation while preserving column boundaries. This yields an interpretable hierarchy and efficient runtime. Classic XY-cut and whitespace-based methods inform the splitting criteria~\cite{nagy1986document,breuel2002two}, and Otsu's adaptive binarization is used for robustness~\cite{shafait2008efficient}.
...
The problem is abstracted as follows:
...
The divide-and-conquer algorithm consists of four main phases: (1) recursive XY-cut splitting with feature-guided valley detection, (2) region labeling using edge density, content density, and periodicity features, (3) boundary refinement to better align with content, and (4) multi-pass greedy merging with special handling for narrow columns. The main algorithm and key subroutines are described below. The implementation is available in the modular \texttt{dla/src/} codebase.
...
The core recursive procedure partitions a rectangular region based on feature homogeneity. Projection profiles are computed to identify whitespace valleys (gaps between content blocks), and edge density is used to assess region uniformity. Regions are split where these features indicate inhomogeneity, continuing until each subregion is sufficiently uniform.
...
\textbf{ProjectionValleys:} For axis $a \in \{0,1\}$, compute the projection profile $P[i] = \sum_j (255 - I_{bin}[i,j])$ (summing along the perpendicular axis), which measures content density along that axis. Normalize to $[0,1]$. Valley detection uses adaptive thresholds that adjust to local content characteristics: the threshold is computed as $\max(0.1, \mu - \sigma)$ where $\mu$ and $\sigma$ are the mean and standard deviation of the normalized profile. This adapts to sparse content (lower threshold) versus dense content (higher threshold), making the algorithm robust to varying document densities. The algorithm prioritizes large gaps ($\geq 40$px) as semantic separators (column boundaries) over small line gaps, but falls back to all detected valleys if no large gaps are found. This adaptive behavior handles both dense multi-column layouts and sparse single-column documents.
...
\textbf{FeatureStop:} Return true if $w < min\_size$ or $h < min\_size$ or edge density $< 0.01$ (region is sufficiently uniform, mostly blank or homogeneous). The edge density is computed using Canny edge detection. The minimum size threshold $min\_size$ is adaptively set based on image dimensions: $min\_size = \max(min\_region, \min(W, H) / 100)$, ensuring that the stopping criterion scales appropriately with image resolution. This adaptive sizing prevents over-segmentation on high-resolution images while maintaining fine-grained segmentation on smaller images. The threshold ensures we stop when further splitting would not improve segmentation quality.

\subsection{Labeling Heuristic}
Each region is classified using multiple features computed on a cropped region (with margins removed to avoid edge artifacts). The classification uses adaptive thresholds that consider region size and aspect ratio to handle edge cases:

\begin{itemize}
    \item \textbf{Edge density:} $\rho = \text{mean}(\text{Canny}(roi, 50, 150))$ computed on the cropped region
    \item \textbf{Content density:} $\delta = \text{count}(\text{dark pixels}) / \text{total pixels}$ using Otsu's adaptive thresholding, which automatically determines the optimal threshold for each image
    \item \textbf{Periodicity:} $p_h, p_v = \text{FFTPeakiness}(\text{projections})$ computed on downsampled projections. For a projection $P$, $\text{FFT}(P - \bar{P})$ is computed, magnitude is taken, and peakiness is calculated as $\max(\text{mag}[2:\text{len}/2]) / \text{mean}(\text{mag}[2:\text{len}/2])$ (excluding DC component).
    ...
    \item \textbf{Size and aspect ratio:} Region area and aspect ratio are used to distinguish figures (moderate size, square-ish) from text (variable size, often tall or wide) and tables (grid structure).
\end{itemize}

Classification rules (applied in order, with size-adaptive thresholds):
\begin{enumerate}
    \item \textbf{Blank:} If $\delta < 0.005$ and $\rho < 0.01$, or if region is very small ($\text{area} < 5000$) with $\delta < 0.03$ and $\rho < 0.02$. The size-adaptive threshold prevents misclassifying small content regions as blank.
    \item \textbf{Figure:} If $\rho > 0.12$ and region has appropriate size ($40,000 \leq \text{area} \leq 800,000$) and aspect ratio ($0.5 \leq \text{aspect ratio} \leq 4.0$). Size constraints prevent misclassifying very small or very large regions as figures. This check occurs before table detection to handle figures with grid-like elements.
    \item \textbf{Table:} If both horizontal and vertical edge lines are detected ($\geq 3$ lines each) with balanced edge distribution, OR if $p_h > 2.5$ and $p_v > 2.5$ (strong periodicity in both directions). Additional size and aspect ratio checks ensure tables are appropriately sized.
    \item \textbf{Text:} If $0.02 \leq \rho \leq 0.12$ and $\delta > 0.01$, with size constraints ($5,000 \leq \text{area} \leq 150,000$ for moderate-sized text, or appropriate aspect ratios for tall/wide text blocks). The adaptive thresholds handle both dense and sparse text regions.
    \item \textbf{Default:} Label as \text{text} if none of the above conditions are met and content density $> 0.02$, otherwise label as \text{blank}.
\end{enumerate}

The adaptive thresholds based on region size, aspect ratio, and content characteristics help handle edge cases such as small text fragments, large merged regions, and ambiguous layouts.

\subsection{Greedy Merge}
The merge phase consists of two steps: (1) greedy merging of adjacent regions, and (2) merging small adjacent regions in the same column. This post-processing step addresses over-segmentation that can occur when the recursive splitting creates too many small regions.
...
For ambiguous regions where labels might be uncertain (e.g., a region with mixed text and table characteristics), we prioritize merging with regions that have higher confidence labels. Same-column regions use increased padding ($1.5p$) to merge text regions more aggressively, as text blocks in the same column are likely part of the same semantic unit. For narrow columns ($w < 80$px), we are conservative about horizontal merging to avoid merging separate columns, which could combine distinct content blocks.

\textbullet \textbf{Step 2 - Merge Small Adjacent:} After initial merging, we merge very small adjacent regions ($\text{area} < 10,000$ pixels) that are in the same column, using padding $p = 30$. This helps fix over-segmentation where many tiny regions were created, particularly for regions that were split too finely during recursion. Small regions are merged if they share compatible labels and are vertically aligned, ensuring we combine fragments that likely belong to the same content block.

\textbf{Boundary Refinement:} Before merging, region boundaries are refined to better fit content by shrinking boundaries to remove whitespace margins. This refinement step scans the perimeter of each region and contracts boundaries until they encounter content pixels, improving alignment with actual content and reducing false positives from edge artifacts.

\section{Proof of Correctness}

Three properties are proven: termination, validity (output is a partition), and soundness of the stopping criterion.

\subsection{Termination}
\textbf{Proposition.} The XY-CUT procedure terminates for any input image $I$ of size $N = H \times W$ pixels.

\textbf{Pf.} Every recursive path is finite. Consider recursion depth $d$ and region dimensions $(w, h)$.
...
\textbf{Pf.} The proof proceeds by structural induction on the recursion tree.
...
At each node, the following operations are performed:
...
\textbullet Labeling each of $k$ regions takes $O(k \cdot \bar{A})$ where $\bar{A}$ is average region area. For each region, the following are computed: Canny edges $O(\bar{A})$, FFT on downsampled projections $O(\max(w, h) \log \max(w, h))$, and edge structure analysis $O(\bar{A})$. Since regions partition the image, $\sum_i A_i = N$, and the FFT term is dominated by $O(\bar{A})$ for typical regions, total labeling is $O(N)$.
...
\section{Algorithm Explanation in Domain Language}
...
\textbf{Step 2: Recursive Splitting.} Starting with the entire page, natural boundaries between content blocks are identified. Projection profiles are computed: for each column (vertical projection), the darkness of pixels is summed across all rows. Where the sum is very low, a ``valley'' appears, which is a gap of whitespace that likely separates two content blocks (e.g., the gap between two columns of text). Similarly, for horizontal projections, peaks (high-content areas) are found that indicate row boundaries. 
...
\section{Experimental Evaluation and Runtime Verification}

The algorithm was tested on real document images to validate its performance, and the theoretical analysis is validated through controlled experiments on synthetic document pages with known ground truth.

\subsection{Experimental Setup}
The algorithm was tested on both real document images and synthetic data. Real-world testing was performed on diverse scanned document images including academic papers, multi-column articles, and forms to validate the algorithm's performance on actual documents with varying layouts, fonts, and image quality. These real-world tests confirmed that the algorithm successfully segments complex layouts, handles varying column structures, and correctly identifies text, table, and figure regions in practice.

For quantitative evaluation and runtime analysis, synthetic multi-column pages are generated using a synthetic data generator that creates pages with known ground-truth regions. The generator randomly places 1-3 columns, each containing text blocks (simulated as horizontal lines), tables (grid structures), figures (noise + random rectangles), and blank regions. Testing is performed at multiple resolutions: 266$\times$400, 600$\times$400, 900$\times$600, 1200$\times$800, and 1500$\times$1000 pixels. Between 30-220 pages are generated per resolution to ensure statistical significance. Ground-truth bounding boxes and labels are known for synthetic data, enabling accurate quantitative evaluation and runtime measurement. The synthetic data allows systematic testing across different resolutions and layout complexities, while real-world testing validates practical applicability.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{runtime_loglog.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{accuracy_vs_resolution.png}
...
While the algorithm uses adaptive thresholds (based on content density, region size, and image resolution) and handles many edge cases through size-aware classification and adaptive merging, some highly complex or unusual layouts may still pose challenges. The approach may struggle with documents containing non-standard layouts (e.g., circular text, extreme aspect ratios), heavily degraded images with significant noise, or ambiguous region boundaries where multiple interpretations are valid. The current rule-based classification, while adaptive to size and content characteristics, does not learn from data like deep learning models. Future work includes learned split scoring using small neural networks to better handle unusual split patterns, and robust labeling via small CNNs trained on diverse document layouts to improve classification accuracy on edge cases while maintaining the interpretability of the divide-and-conquer structure.

\section{Conclusion}
This work provides an interpretable D\&C page segmentation algorithm, analyzes its runtime, and validates its empirical behavior on both synthetic and real document images. The algorithm demonstrates good performance on standard document layouts while maintaining interpretability and efficiency. Future work includes learned split scoring and robust labeling via small CNNs to improve handling of edge cases and complex layouts.
...
LLM assistance (ChatGPT) was used during development for the following technical tasks:

\begin{itemize}
    \item \textbf{LaTeX compilation errors:} Fixed LaTeX compilation errors including Unicode character handling, figure placement issues, and BibTeX formatting. The LLM suggested using the \texttt{float} package with \texttt{[H]} placement option and the \texttt{placeins} package for \texttt{\textbackslash FloatBarrier} to resolve figure positioning problems.
    
    \item \textbf{BibTeX formatting:} Added missing required fields (publisher, address) to bibliography entries that were causing compilation warnings.
    
    \item \textbf{ACM template configuration:} Removed copyright notice by adding \texttt{nonacm} option to the document class.
    
    \item \textbf{Algorithm pseudocode syntax:} Assisted with LaTeX algorithm package syntax for formatting the divide-and-conquer pseudocode.
\end{itemize}

The algorithm implementation, mathematical proofs, complexity analysis, experimental design, and all core code logic were developed independently. LLM assistance was used only for LaTeX syntax and formatting issues, not for any algorithmic or theoretical content.

\subsection{Example LLM Prompts and Responses}
Below are representative examples of prompts and responses used during development:

\textbf{Prompt 1:} "LaTeX Error: Unicode character not set up for use with LaTeX. How do I fix Unicode character errors in LaTeX?"

\textbf{Response:} The LLM suggested replacing Unicode characters with LaTeX commands, such as using \texttt{\textbackslash textbullet} for bullet points and \texttt{\$\textbackslash square\$} in math mode for square symbols. Also recommended adding \texttt{\textbackslash usepackage\{amssymb\}} for special symbols.

\textbf{Prompt 2:} "My figures are overlapping with text in LaTeX. How can I fix figure placement?"

\textbf{Response:} The LLM recommended using the \texttt{float} package with \texttt{[H]} placement option to force figures exactly where placed in the code, and \texttt{\textbackslash FloatBarrier} from the \texttt{placeins} package to prevent float overlap.

\textbf{Prompt 3:} "BibTeX warning: empty publisher field. What fields are required for @inproceedings entries?"

\textbf{Response:} The LLM explained that \texttt{@inproceedings} entries require \texttt{publisher} and \texttt{address} fields. Provided examples showing how to add these fields, such as \texttt{publisher=\{Springer\}} and \texttt{address=\{Berlin, Germany\}} for conference proceedings.

These examples represent the type of assistance received. All suggestions were reviewed, tested, and modified as needed before inclusion in the final paper.

\section{Code Appendix}
All source code used to validate runtime analysis (segmentation, synthetic data, evaluation, experiments) is available online at:

\texttt{https://github.com/cherrymanu/dla-dc-segmentation}

The repository includes the \texttt{dla/} directory (main implementation) and \texttt{code/} directory (evaluation utilities). The modular implementation in \texttt{dla/src/} clearly separates the algorithm components: preprocessing, XY-cut recursion, labeling, and merging. Plots referenced in the paper are produced by \texttt{dla/run\_experiments.py} and saved under \texttt{dla/outputs/plots/}.
